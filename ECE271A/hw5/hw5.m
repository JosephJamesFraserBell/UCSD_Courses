...
num_mixtures = 1;
num_components = 32; %{1, 2, 4, 8, 16, 32}
%8 components part
...
for mix=1:num_mixtures
    ...
    for i=1:num_components   
        ...
        covariance_c_FG = [covariance_c_FG; abs(normrnd(3,0.01,[1,64]))]; %random covariance
...
for mix=1:num_mixtures
    ...
    for i=1:num_components
        ...
        covariance_c_BG = [covariance_c_BG; abs(normrnd(3,0.01,[1,64]))]; %random covariance
        weights = [weights, 1/num_components]; %giving all initial equal weights
    end
        %gaussian function - taken from my last homework
...
prob_error = error_storage('1');
figure()
plot(dimensions, prob_error, 'r--o');
xlabel('Number of Dimensions');
ylabel('Probability of Error');
title(['Number of Components: ' num2str(num_components)]);
