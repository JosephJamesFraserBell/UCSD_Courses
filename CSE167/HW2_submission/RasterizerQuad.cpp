...
void RasterizerQuad::rasterize(glm::mat4 M, glm::mat4 V, glm::mat4 P, glm::mat4 D, PointCloud& obj) {
	...
	glm::vec4 p;
	glm::vec4 pprime;
	std::vector<glm::vec3> points = obj.points;
	std::vector<glm::vec3> normals = obj.normals;
	...
	{
		p = glm::vec4(points[i], 1.0);
		new_normal = 0.5f * normalize(normals[i]) + 0.5f;
		xn = new_normal[0];
		yn = new_normal[1];
		zn = new_normal[2];
		...
		if (n > 1) {
			for (int j = 0; j < n; j++) {
				drawPoint(pprime[0] + j, pprime[1] + j, pprime[2], xn, yn, zn);
				drawPoint(pprime[0], pprime[1] + j, pprime[2], xn, yn, zn);
				drawPoint(pprime[0] + j, pprime[1], pprime[2], xn, yn, zn);
				drawPoint(pprime[0], pprime[1], pprime[2], xn, yn, zn);
			}
		}
		else {
			drawPoint(pprime[0], pprime[1], pprime[2], xn, yn, zn);
